# Title     : Causal forest with inversions, construction
# Objective : Train a causal forest using only construction data, with inversion as instruments
...
encoder_x <- mydata %>%
  select(tot_emplvl, raw_prcp, raw_tmax, raw_tmin, wind_speed) %>%
  as.matrix()
